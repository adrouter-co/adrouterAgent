...
This is the operator runbook for `v0.1.0-beta.7`. It separates safe repository
...
git add .
git commit -m "Prepare AdRouter Agent 0.1.0-beta.7 public release"
...
git switch main
git pull --ff-only
test "$(node -p "require('./package.json').version")" = "0.1.0-beta.7"
...
Do not attempt to republish `0.1.0-beta.7` to test OIDC. npm versions are
