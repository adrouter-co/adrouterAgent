...
const createEnvelope = (overrides = {}) => {
  ...
  const version = overrides.releaseVersion ?? '0.1.0-beta.13';
  const signed = {
...
test('fixed-origin update checks reject redirects and accept only a trusted matching channel', async () => {
  ...
  assert.equal(result.latestVersion, '0.1.0-beta.13');
...
test('expired unhealthy activation restores the prior managed installation', async () => {
  ...
  try {
    ...
    await writeFile(
      paths.receiptPath,
      JSON.stringify({ ...previousReceipt, releaseVersion: '0.1.0-beta.13' })
    );
    await writeFile(
      paths.pendingPath,
      JSON.stringify({
